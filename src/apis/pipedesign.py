...
api = Namespace(
    "pipedesign", description="Namespace holding all methods related to pipedesigns."
)

pipedesign_model = api.model(
    name="Pipedesign model",
    model={
...
class Pipedesign(Resource):
    ...
    def get(self, pipedesign_id):
        ...
        result = handler.azure_blob_to_json(
            container_name=current_app.config["CONTAINER_NAME_DATA"],
            blob_name=pipedesign_id,
        )
