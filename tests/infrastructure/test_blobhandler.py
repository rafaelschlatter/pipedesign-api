...
class TestBlobHandler():
    ...
    def test_azure_blob_to_json_failure(self):
        handler = blobhandler.BlobHandler()
        result = handler.azure_blob_to_json(container_name=os.environ["CONTAINER_NAME_DATA"], blob_name="non_existant_blob_name")
    ...
    def test_json_to_azure_blob_failure(self):
        handler = blobhandler.BlobHandler()
        pipedesign_json = HelperFunctions._load_json()
        result = handler.json_to_azure_blob(container_name="invalid_container_name", pipedesign_json=pipedesign_json)
        assert result[0] == False
    ...
    def test_azure_blob_to_model_success(self):
        handler = blobhandler.BlobHandler()
        result = handler.azure_blob_to_model(model_id="test_model_1_do_not_delete",
    ...
    def test_azure_blob_to_model_failure(self):
        handler = blobhandler.BlobHandler()
        result = handler.azure_blob_to_model(model_id="non_existant_model_id",
            container_name=os.environ["CONTAINER_NAME_MODELS"])
        assert result[0] == False
    ...
    def test_model_to_azure_blob_success(self):
        ...
        handler = blobhandler.BlobHandler()
        result = handler.model_to_azure_blob(model=model, container_name=os.environ["CONTAINER_NAME_MODELS"],
    ...
    def test_model_to_azure_blob_failure(self):
        model = HelperFunctions._create_test_model()
        handler = blobhandler.BlobHandler()
        result = handler.model_to_azure_blob(model=model, container_name="non_existant_container")
        assert result[0] == False
        assert isinstance(result[1], AzureHttpError)
